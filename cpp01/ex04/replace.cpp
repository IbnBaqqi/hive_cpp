...
bool is_valid(int ac, char **av)
{
	if (ac != 4) {
		...
		std::cerr << "Usage: ./replace <file_name> <string_to_be_replaced> <new_string>" << std::endl;
		return false;
	}
...
bool is_fstreams_ready(const std::string file_name, std::fstream *fin, std::fstream *fout)
{
...
bool replace(std::fstream *fin, std::fstream *fout, const std::string s1, const std::string s2)
{
